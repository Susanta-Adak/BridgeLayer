...
offline access token, mirroring the Zoho auth module's shape.
Unlike Zoho, Shopify requires two extra checks on the callback that
have no Zoho equivalent: an HMAC signature over the query string
(proves the redirect came from Shopify, using the app's client
secret) and a `state` nonce (defends against CSRF). Both are
enforced here before a code is ever exchanged.
...
async def exchange_code_for_token(shop: str, code: str) -> Token:
    settings = get_settings()
...
def load_token() -> Token | None:
    with SessionLocal() as db:
        return db.query(Token).filter(Token.provider == "shopify").first()


def _save_token(payload: dict) -> Token:
    with SessionLocal() as db:
        token = db.query(Token).filter(Token.provider == "shopify").first()
        if token is None:
            token = Token(provider="shopify", access_token="")
            db.add(token)
