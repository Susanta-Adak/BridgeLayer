...
Tokens are persisted to the `tokens` table (provider="zoho") so the
process can restart without forcing the user to re-authorize.
"""
...
async def exchange_code_for_token(code: str) -> Token:
    settings = get_settings()
...
def load_token() -> Token | None:
    with SessionLocal() as db:
        return db.query(Token).filter(Token.provider == "zoho").first()


def _save_token(payload: dict, *, require_refresh_token: bool) -> Token:
    expires_in = payload.get("expires_in", 3600)
    ttl = datetime.timedelta(seconds=expires_in)
    expires_at = utcnow() + ttl - _EXPIRY_SAFETY_MARGIN

    with SessionLocal() as db:
        token = db.query(Token).filter(Token.provider == "zoho").first()
        if token is None:
            token = Token(provider="zoho", access_token="")
            db.add(token)
